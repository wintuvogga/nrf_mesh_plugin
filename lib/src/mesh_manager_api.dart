...
class MeshManagerApi {
  ...
  late final _onVendorMessageStatusController =
      StreamController<VendorModelStatusData>.broadcast();
  ...
  late StreamSubscription<VendorModelStatusData>
      _onVendorMessageStatusSubscription;
  ...
  MeshManagerApi() {
    ...
    _onVendorMessageStatusSubscription = _eventChannelStream
        .where((event) =>
            event['eventName'] == MeshManagerApiEvent.vendorMessageStatus.value)
  ...
  Stream<VendorModelStatusData> get onVendorMessageStatus =>
      _onVendorMessageStatusController.stream;
  ...
  /// Will send a get GenericLevelOnOff message to the given [address].
  Future<GenericOnOffStatusData> sendGenericOnOffGet(int address,
      {int keyIndex = 0}) async {
    final status = _onGenericOnOffStatusController.stream.firstWhere(
      (element) => element.source == address,
      orElse: () => const GenericOnOffStatusData(-1, false, false, -1, -1),
    );
    await _methodChannel.invokeMethod('sendGenericOnOffGet', {
      'address': address,
      'keyIndex': keyIndex,
    });
    return status;
  ...
  // Will send a acknowledged vendor model message to the given [address].
  ...
  // Will send a unacknowledged vendor model message to the given [address].
